...
Chip8::Chip8() {
  this->regReset();
  fontMap = {0xF0, 0x90, 0x90, 0x90, 0xF0, 0x20, 0x60, 0x20, 0x20, 0x70,
...
Chip8::~Chip8() { this->gamestate = false; }
...
void Chip8::opCycle(const uint16_t& op) {
  switch (op & OPMASK) {
    ...
    case 0x8000:
      switch (op & 0x000F) {
        ...
        case 0x0004:
          ...
          pc += 2;
          break;
        /* 8XY5 - Subtract the value of VY from VX. VF = 00 if borrow, VF = 01
         * if not. */
        case 0x0005:
          ...
          pc += 2;
          break;
        /* 8XY6 - Store VY shifted right by one bit in VX. VF = LSB of VX prior
         * to shift. */
        case 0x0006:
        ...
        case 0x0007:
          // V[(op & 0x0F00) >> 8] -= (V[(op & 0x00F0) >> 4] - V[(op & 0x0F00)
          // >> 8]); V[(op & 0x0F00) >> 8] > V[(op & 0x00F0) >> 4] ? V[0xF] =
          // 0x00 : V[0xF] = 0x01;
          ...
          pc += 2;
          break;
        /* 8XYE - Store VY left shifted one bit to VX. Set VF to the MSB of VY
         * prior to the shift. */
        case 0x000E:
          // V[(op & 0x0F00) >> 8] = V[(op & 0x00F0) >> 4] >> 1;
    ...
    case 0xE000:
      switch (op & 0x00FF) {
        /* EX9E - Skip following instr if the key being pressed == key hex code
         * in VX */
        case 0x009E:
          if (key[V[(op & 0x0F00) >> 8]] != 0)
            ...
            pc += 2;
          break;
        /* EXA1 - Skip following instr if the key being pressed != key hex code
         * in VX */
        case 0x00A1:
          if (key[V[(op & 0x0F00) >> 8]] == 0)
            pc += 4;
    ...
    case 0xF000:
      switch (op & 0x00FF) {
        ...
        case 0x0055:
          for (int i = 0; i <= ((op & 0x0F00) >> 8); ++i) ram[I + i] = V[i];
        ...
        case 0x0065:
          for (int i = 0; i < ((op & 0x0F00) >> 8); ++i) V[i] = ram[I + i];
...
const uint16_t Chip8::fetchOpcode() { return ram[pc] << 8 | ram[pc + 1]; }

const bool Chip8::getGameState() { return gamestate; }
